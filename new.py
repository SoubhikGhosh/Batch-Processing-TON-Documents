...
# Increased thread pool size for parallel processing
MAX_WORKERS = 20  # Increase this based on your server's capacity
# Batch size for document processing
BATCH_SIZE = 10  # Adjust based on your API limits and memory constraints
# ============================================================
        ...
        for zip_index, (zip_content, zip_name) in enumerate(zip(file_contents, file_names)):
            ...
            for folder_name, files_list in folder_files.items():
                ...
                for i in range(0, len(files_list), BATCH_SIZE):
                    ...
                    # Process the batch
                    batch_results = DocumentProcessor.process_document_batch(batch)
...
async def upload_files(
    ...
    try:
        ...
        return {
            "status": "processing",
            "job_id": job_id,
            "message": "Files uploaded successfully. Processing started with parallel and batch processing.",
            "files": file_names
        }
...
if __name__ == "__main__":
    ...
    uvicorn.run("new:app", host="0.0.0.0", port=8080, reload=True)